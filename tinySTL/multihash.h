...
namespace tinySTL
{
	...
	public:
		...
		multimap() :_tree(Compare()) {}
		explicit multimap(const Compare& cmp) :_tree(cmp) {}

		template<typename _InputIterator>
		multimap(_InputIterator first, _InputIterator last) { _tree.insert_equal(first, last); }

		template<typename _InputIterator>
		multimap(_InputIterator first, _InputIterator last, Compare cmp)
			: _tree(cmp) {
			_tree.insert_equal(first, last);
		}

		multimap(const multimap<Key, T, Compare, Alloc>& s) :_tree(s._tree) {}
		multimap<Key, T, Compare, Alloc>& operator=(const multimap<Key, T, Compare, Alloc>& s)
		{
			_tree = s._tree;
			return *this;
		}
		...
		void swap(multimap<Key, T, Compare, Alloc>& m) { _tree.swap(m._tree); }

		pair<iterator, bool> insert(const value_type& x) { return _tree.insert_equal(x); }
		iterator insert(iterator pos, const value_type& x) { return _tree.insert_equal(pos, x); }
		template<typename _InputIterator>
		void insert(_InputIterator first, _InputIterator last) { _tree.insert_equal(first, last); }
